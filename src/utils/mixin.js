  ...
  Editor: {
    data() {
      return {
        ...
        disMenu: [], // 右键菜单
        menuList: {
          Delete: '删除', 'Ctrl+c': '复制', 'Ctrl+up': '置顶', 'Ctrl+down': '置底',
        },
        menuStyle: {
          zIndex: 1000, visibility: 'hidden', left: 0, top: 0,
        }, // 右键菜单样式
        activeName: 'config',
        print: [],
        config: {
          ...
          page: {
            ...
            rules: {
              width: {
                type: 'number', required: true, message: '请设置模板宽度', trigger: 'change',
              },
              height: {
                type: 'number', required: true, message: '请设置模板高度', trigger: 'change',
              },
    ...
    methods: {
      ...
      saveXML() {
        ...
        string = string.replace(/(start|end)([xy])=/g, (a, b, c) => `${b + c.toUpperCase()}=`);
      ...
      convertXML(xml) {
        ...
        // 处理字符串
        const xmlString = xml.replace(/(["[])<%=/gm, '$1&lt;%=').replace(/%>(["\]])/gm, '%&gt;$1').replace(/<%[\s\S]*?%>/gm, '');
      ...
      handleDropDown(c) {
        if (this.typeArray.includes(c)) {
          ...
          const obj = Object.assign(_obj.config, this.CONFIG[c.replace(/^[vh]-/, '')]);
          if (c.includes('text')) {
      ...
      handleContextMenu(type) {
        if (this.getActiveLayoutObj) {
          ...
          switch (type) {
                ...
                if (obj.active) {
                  if (this.store) {
                    this.print.forEach((o) => {
                      ...
                      l >= 0 && o.list.findIndex(k => k.name === obj.name &&
                      !k.disabled) >= 0 && o.value.splice(l, 1);
                    });
                  } else {
            ...
            case 39:// right
            case 40:// down
              const con = this.config[this.getConfigType(this.getActiveLayoutObj)];
